...
export @sswitch, @smatch
using .AbstractPatterns
using .AbstractPatterns.BasicPatterns
...
basic_ex2tf(eval::Function, a) = literal(a)
function basic_ex2tf(eval::Function, s::QuotePattern)
    p0 = P_type_of(QuoteNode)
    p1 = decons(_quote_extract, [basic_ex2tf(eval, s.value)])
    and([p0, p1])
end
    ...
    elseif hd === :call
        let f = args[1], args′ = view(args, 2:length(args))
            ...
            if t == Expr
                ...
                args′ = Any[args′[1], Expr(:vect, view(args′, 2:n_args′)...)]
                n_args′ = 2
            elseif t === Core.svec
                tag, split = ellipsis_split(args′)
...
"""a minimal implementation of @match
"""
macro smatch(val, tbl)
...
function gen_match(val, tbl, __source__::LineNumberNode, __module__::Module)
    ...
    for i in eachindex(tbl.args)
        ...
        @sswitch stmt begin
            ...
            k += 1
            pattern = try
                basic_ex2tf(__module__.eval, case)
            catch e
                e isa ErrorException && throw(PatternCompilationError(ln, e.msg))
                rethrow()
            end
